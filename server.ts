...
// Helper middleware for Authentication (Supports both Supabase Auth and Local JWT)
    ...
    try {
      ...
      if (sbUser && !error) {
        ...
        let user = db.users.find(
          (u) =>
            (u.email && sbUser.email && u.email.toLowerCase() === sbUser.email.toLowerCase()) ||
            u.usuario.toLowerCase() === (sbUser.user_metadata?.username || sbUser.email?.split('@')[0] || '').toLowerCase()
        );
        ...
        if (!user) {
          ...
          user = {
            ...
            nome: sbUser.user_metadata?.name || sbUser.user_metadata?.username || sbUser.email?.split('@')[0] || 'Usuário',
            usuario: sbUser.user_metadata?.username || sbUser.email?.split('@')[0] || `user${Date.now()}`,
            email: sbUser.email,
            senhaHash: '',
...
app.post('/api/auth/google', (req: Request, res: Response) => {
  ...
  if (!user) {
    const baseUser = cleanEmail.split('@')[0].replace(/[^a-zA-Z0-9]/g, '') || 'googleuser';
